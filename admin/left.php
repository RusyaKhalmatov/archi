
  <h3 style='text-align: center;'>Welcome to admin's page</h3>
  <ul class="nav nav-pills nav-stacked">
    <li class="active" style="text-align: center;"><a href="../index.php">See the web site</a></li>
    <li class="dropdown" style="//text-align: center;">
      <a class="dropdown-toggle" data-toggle="dropdown" href="#">Admin's settings <span class="caret"></span></a>
      <ul class="dropdown-menu">
        <li><a href="admsettings/adminList.php">List of admins</a></li>
        <li><a href="admsettings/adm-profile.php">Profile settings</a></li>
        <li><a href="admsettings/add_admin.php">Add admin</a></li>                        
      </ul>
    </li>
    <li><a href="#">Orders<span class="badge">5</span></a></li>
    <li><a href="#">Mail<span class="badge">5</span></a></li>
    <li><a href="add_order.php">Add order</a></li>
    <li><a href="#">Users</a></li>
    <li><a href="statistics.php">Statistics</a></li>
  <!--li><a href="../check.php">Check password</a></li-->